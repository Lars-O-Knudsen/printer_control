...
class request:
    ...
    async def read_headers(self, save_headers=[]):
        ...
        \r\n
        """
        sh_upper = [h.upper() for h in save_headers]
        while True:
            gc.collect()
            ...
            if len(frags) != 2:
                raise HTTPException(400)
            if frags[0].upper() in sh_upper:
                self.headers[frags[0].upper()] = frags[1].strip()
    ...
    def contextParam(self, key:str) -> any:
        """Return server context parameter or None if not found"""
        return self.server.context.get(key,None)
...
class webserver:
    ...
    def __init__(self, request_timeout=3, max_concurrency=3, backlog=16, debug=False, context={}):
        ...
        Keyword arguments:
            ...
            context           - a server context that can be used to pass down
                              parameters to handlers through request.server reference
        """
    ...
    async def _handle_request(self, req, resp):
        ...
        req.server = self  # pass down server reference
        if not req.handler:
