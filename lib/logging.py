...
def getContextLogger(context:str, base=None):
    if context in _loggers:
    ...
    ctx = _loggers[context] = Logger(name=context)
    if base in _loggers:
        ctx.handlers = _loggers[base].handlers
        ctx.level = _loggers[base].level
    return ctx
