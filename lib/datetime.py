...
"""
    ...
    taken from ntptime_tz
"""
def eu_dst_offset(dt): 
    cur = time.gmtime(dt)
    if cur[1] == 3 or cur[1] == 10:
...
class tzinfo:
    ...
    """
    @classmethod
    def timezone_offset(cls,tz_str: str) -> int:
        try:
            s:str=tz_str.upper()
            if s in ["CET","COPENHAGEN","BERLIN","PARIS"]:
                s = "-1"
            s = s.replace(' ', '')
            s= s.replace('GMT','')
            if s == '': 
...
class timezone(tzinfo):
    ...
    @classmethod
    def fromName(cls, tzname:str = "GMT"):
        offset_s = cls.timezone_offset(tzname)
        return timezone(timedelta(seconds=offset_s),tzname)
...
class time:
    def __init__(self, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.UTC(), *, fold=0):
...
class datetime: 
    def __init__(
        self, year, month, day, hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.UTC(), *, fold=0
    ):
        self._d = _date(year, month, day)
        self._t = _time(hour, minute, second, microsecond, fold)
        self._tz = tzinfo if tzinfo != None else timezone.UTC()
        self._fd = fold

    @classmethod
    def fromtimestamp(cls, ts, tz=timezone.UTC()):
    ...
    @classmethod
    def combine(cls, date, time, tzinfo=timezone.UTC()):
        return cls(
